...
-- 複合インデックス（検索パフォーマンス向上用）
CREATE INDEX IF NOT EXISTS idx_company_schedules_active_date ON company_schedules(is_active, start_date);
CREATE INDEX IF NOT EXISTS idx_company_schedules_active_type ON company_schedules(is_active, schedule_type);
CREATE INDEX IF NOT EXISTS idx_company_schedules_holiday_date ON company_schedules(is_holiday, start_date) WHERE is_holiday = true;
