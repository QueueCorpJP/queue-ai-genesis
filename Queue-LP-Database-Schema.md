...
- **今日の出勤者データ取得機能**
  ...
  - 開始時間順でソート表示、レスポンシブ対応（モバイル1列・デスクトップ3列）

- **時間表示フォーマット改善**
  - formatTime() 関数追加（HH:MM形式で時間表示）
  - getAttendanceTypeIcon/Label() 関数追加（勤務形態のアイコン・ラベル変換）
  - 勤務時間・勤務形態・ステータスの視覚的表示強化

### 2025年1月11日 - Admin勤怠編集機能追加
- **管理者権限による勤怠データ編集機能**
  - AdminCalendarOverview.tsx にadmin・executive・CEO向け勤怠編集機能追加
  - 今日の出勤者カードと勤怠テーブルの各行に「編集」ボタン追加
  - 他のメンバーの勤怠時間・ステータス・勤務形態を変更可能

- **勤怠編集モーダル機能**
  - 開始時刻・終了時刻の時間入力（HH:MM形式）
  - 休憩時間（分単位）・労働時間自動計算
  - ステータス選択（予定・出勤・欠勤・遅刻・早退）
  - 勤務形態選択（通常勤務・リモート・出張・病気休暇・有給休暇）
  - 備考テキストエリア（自由入力）

- **管理者権限とセキュリティ**
  - user.role が 'admin'・'executive'・'ceo' のみ編集機能利用可能
  - getSupabaseAdmin() による管理者クライアント使用
  - 更新後の自動データ再取得・今日の出勤者リスト同期
  - エラーハンドリング・成功通知・操作ログ記録

### 2025年2月3日 - KPI/KGI管理システム複数測定単位対応・案件ベース測定機能追加

**KPI/KGI複数測定単位対応**:
- **1つの指標に複数の測定単位設定**: 売上指標で「金額」「件数」「達成率」など複数単位を同時管理
- **測定単位タイプ分類**: count（数量）・amount（金額）・percentage（割合）・ratio（比率）・time（時間）・score（スコア）
- **主要単位指定**: 指標あたり1つの主要単位を設定、ダッシュボード表示での優先表示
- **単位別目標設定**: 各測定単位に個別の目標値・期間・担当者を設定可能
- **単位別進捗記録**: 測定単位ごとに独立した進捗記録・達成率計算
- **複数単位モード切替**: 標準モードと複数単位モードをUI上で切り替え可能

**案件ベース測定機能**:
- **測定頻度選択肢拡張**: 従来の日次/週次/月次/四半期/年次に加え「案件ごと」オプションを追加
- **案件ベース測定機能**: プロジェクト案件と連携したKPI測定が可能
- **案件選択インターフェース**: 案件ごと測定選択時に関連案件を選択可能なUIを実装
- **進捗記録連携**: 案件IDと紐づけた進捗記録機能（project_case_idフィールド追加）
- **データベース連携**: project_casesテーブルとの連携によるアクティブ案件自動取得

**実装詳細**:
- **複数測定単位対応**: 
  - 新テーブル: kpi_measurement_units（測定単位管理）
  - 新テーブル: kpi_multi_unit_targets（複数単位別目標管理）
  - 新テーブル: kpi_multi_unit_progress_records（複数単位別進捗記録）
  - 新ビュー: kpi_indicators_with_units（指標と測定単位統合）
  - 新ビュー: kpi_multi_unit_targets_detail（複数単位目標詳細）
  - 新ビュー: kpi_multi_unit_progress_summary（進捗統合）
  - 新ビュー: kpi_multi_unit_dashboard_stats（統計ダッシュボード）
- **UI機能**:
  - 複数単位モード切替ボタン（標準⇔複数単位）
  - 動的測定単位追加・削除フォーム
  - 主要単位選択機能（1指標1主要単位制約）
  - 単位タイプ分類選択（数量・金額・割合・比率・時間・スコア）
- **案件ベース機能**:
  - KPIManager.tsx: 測定頻度型定義拡張（'project-based'追加）
  - 進捗記録フォーム: 案件ごと測定時の案件選択UI追加
  - 頻度ラベル変換関数: getFrequencyLabel()実装で日本語表示対応
  - 案件データ管理: fetchProjectCases()関数でアクティブ案件取得
  - 進捗記録データ: project_case_idフィールド追加でトレーサビリティ向上

**対象ユーザー**: 全ロール（複数測定単位・案件に応じたKPI測定が可能）
**データベース影響**: 
- **新規テーブル**: kpi_measurement_units, kpi_multi_unit_targets, kpi_multi_unit_progress_records
- **新規ビュー**: kpi_indicators_with_units, kpi_multi_unit_targets_detail, kpi_multi_unit_progress_summary, kpi_multi_unit_dashboard_stats
- **既存テーブル拡張**: kpi_progress_records に project_case_id カラム追加（UUID型、project_cases外部キー）
- **制約**: 指標あたり主要単位1つまで、単位タイプ制限、外部キー制約
**UI改善**: 複数単位モード切替・動的フォーム・案件選択ドロップダウン・頻度ラベル日本語化・条件付きフィールド表示

---

*最終更新: 2025年2月3日（KPI/KGI複数測定単位対応・案件ベース測定機能追加・Admin勤怠編集機能追加・カレンダービュー今日の出勤者表示機能追加・個人KPI作成機能追加・RLS権限体系見直し）*  
*実装完了: ニュース・ブログ管理（SEO最適化・目次機能付き・セッション追跡強化）・勤怠管理（Admin編集機能付き）・スケジュール管理・販管費管理・KPI/KGI管理（複数測定単位・案件ベース測定機能付き）・マイメモ管理・カレンダー今日の出勤者表示・全システム完全実装*  
