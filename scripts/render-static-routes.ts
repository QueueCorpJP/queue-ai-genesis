...
import { marketingRoutes, routeToComponent } from '../src/ssr/routes';
...
async function renderRoute(route: string, template: string) {
	const Component = routeToComponent[route as keyof typeof routeToComponent];
	if (!Component) {
